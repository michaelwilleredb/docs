...
When distributed high-availability configurations are enabled, multiply the number of vCPUs per instance by the number of data nodes configured to calculate the full price for all resources used. You aren't charged for witness nodes or groups in distributed high-availability configurations, only for the infrastructure costs associated with them.
